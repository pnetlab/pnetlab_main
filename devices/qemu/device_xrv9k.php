...
class device_xrv9k extends device_qemu
{
        ...
        for ($i = 0; $i < $quantity; $i++) {
            ...
            if (!isset($this->ethernets[$i])) {
                ...
                if($i == 0 && $this->first_nic != ''){
                    $flag = ' -device '.$this->first_nic.',addr=' . ((int) ($i / 8) + 3) . '.' . ($i % 8) . ',multifunction=on,netdev=net' . $i . ',mac=' . incMac($this->createFirstMac(), $i);
                }else{
                    $flag = ' -device %NICDRIVER%,addr=' . ((int) ($i / 8) + 3) . '.' . ($i % 8) . ',multifunction=on,netdev=net' . $i . ',mac=' . incMac($this->createFirstMac(), $i);
                }
    ...
    public function prepare()
    {
        ...
        if (is_file($this->getRunningPath() . '/startup-config') && !is_file($this->getRunningPath() . '/.configured')) {
            copy($this->getRunningPath() . '/startup-config',  $this->getRunningPath() . '/iosxr_config.txt');
            $isocmd = 'mkisofs -o ' . $this->getRunningPath() . '/config.iso -l --iso-level 2 ' . $this->getRunningPath() . '/iosxr_config.txt';
            exec($isocmd, $o, $rc);
        }
