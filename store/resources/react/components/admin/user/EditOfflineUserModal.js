...
class EditOfflineUserModal extends Component {

	constructor(props, context) {
		super(props, context);
        this.id = makeId();
        this.table = this.context;
        this.table.children['FuncEditOffUser'] = this;
    ...
    setData(rowData){
        this.setState({role: get(rowData[USER_ROLE], '')}, ()=>{
            this.getRole().then(()=>{
                this.rowData = rowData;
                this.usernameInput.setValue(rowData[USER_USERNAME])
                this.passwordInput.setValue('');
                this.roleInput.setValue(rowData[USER_ROLE]);
                this.noteInput.setValue(rowData[USER_NOTE]);
                if(this.statusInput) this.statusInput.setValue(rowData[USER_STATUS]);
                if(this.activeInput) this.activeInput.setValue(rowData[USER_ACTIVE_TIME]);
                if(this.expiredInput) this.expiredInput.setValue(rowData[USER_EXPIRED_TIME]);
                if(this.maxNodeInput) this.maxNodeInput.setValue(rowData[USER_MAX_NODE]);
                if(this.maxNodeLabInput) this.maxNodeLabInput.setValue(rowData[USER_MAX_NODELAB]);
                if(this.workspaceInput){
                    this.workspaceInput.setWorkSpace(rowData[USER_WORKSPACE]);
                }
            })
        })
    }
						...
						<div className="modal-body" style={{ textAlign: 'initial' }}>
							...
							<div style={{ padding:7 }}>
								...
								<div>
                                        ...
                                        <div style={{margin:'10px 0px'}} title="Note"><Input className='input_item_input' ref={c => this.noteInput = c} placeholder='Note' struct={{
                                            [INPUT_TYPE]: 'textarea',
                                            [INPUT_NULL]: true,
                                        }}></Input></div>
                            ...
                            <div className="modal-footer">
								<button type="button" className="btn btn-primary" onClick={() => {this.editUser()}}>{lang("Save")}</button>
	...
	editUser(){
        ...
        this.table.editRow({
            [DATA_KEY] : [{[USER_POD]: this.rowData[USER_POD]}],
            [DATA_EDITOR] : {
...
EditOfflineUserModal.contextType = TableContext;

export default EditOfflineUserModal
