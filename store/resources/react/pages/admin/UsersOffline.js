...
class Users extends Component {
	...
	constructor(props) {
		...
		this.users_struct[STRUCT_EDIT] = {
			[USER_USERNAME]: {
				[EDIT_NAME]: lang(USER_USERNAME),
				[EDIT_TYPE]: 'text',
				[EDIT_NULL]: false,
				[EDIT_MULTI]: false,
			},
			[USER_PASSWORD]: {
				[EDIT_NAME]: lang(USER_PASSWORD),
				[EDIT_TYPE]: 'password',
				[EDIT_MULTI]: false,
			},
			[USER_ROLE]: {
		...
		this.users_struct[STRUCT_ROWS] = {
			[ROW_FUNCS]: (rowData) => {
				if(rowData[USER_OFFLINE] == '0') return []; 
				return [
		...
		this.users_struct[STRUCT_TABLE] = {
			...
			[FLAG_HEAD_ROW]: true,
			[LINK_FILTER]: '/store/public/admin/users/offFilter',
			[LINK_EDIT]: '/store/public/admin/users/offEdit',
			[LINK_ADD]: '/store/public/admin/users/offAdd',
			[LINK_DELETE]: '/store/public/admin/users/offDrop',
